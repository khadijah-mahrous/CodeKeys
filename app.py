...
def dashboard():
    ...
    return render_template('dashboard.html', data=user_data, languages=LANGUAGE_INFO, lesson_counts=lesson_counts)
...
def predict_next():
    ...
    last_two = context[-2:]
    common_patterns = {
        'pr': ('i', 85), 'in': ('t', 80), 'de': ('f', 75), 'fo': ('r', 90),
        'if': (' ', 95), 'el': ('s', 88), 're': ('t', 70), 'co': ('n', 72),
        'fu': ('n', 85), 'va': ('r', 80), 'le': ('t', 78), 'co': ('n', 75),
        'sy': ('s', 90), 'im': ('p', 82), 'cl': ('a', 78), 'wh': ('i', 85),
        'pa': ('s', 70), 'se': ('l', 68), 'tr': ('u', 72), 'no': ('t', 80),
        '. ': ('d', 60), ', ': ('a', 55), '()': ('{', 70), ': ': ('r', 60),
        '\n ': (' ', 85), '  ': (' ', 75), '= ': ('T', 50), '+ ': ('1', 45),
    }
    if last_two in common_patterns:
        char, confidence = common_patterns[last_two]
        return jsonify({"prediction": char, "confidence": confidence})
    return jsonify({"prediction": None, "confidence": 0})
...
def statistics():
    ...
    for lang in ['basics', 'python', 'javascript', 'java']:
        ...
        languages_data[lang] = {
            ...
            'lesson_history': lesson_history[:5]
        }
